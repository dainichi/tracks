...
void check(){ //a,b,c,d represent a translation by vector (a+b+(c-d)sqrt3,c+d+(a-b)sqrt3)
   ...
   if(a==0 && b==0 && c==0 && d==0){ //check if there are lexically smaller mirror images
      int index = 0,i = total-1,status = EQ;
      for(; i >= 0; i--){
	 if(perm[i] < perm[index]) {status = GT; break;}
	 if(perm[i] == perm[index]) index++; else index=0;
      }
      if(status != GT){
	 i = total-1;
	 for(; index < total; index++){
	    if(perm[i] < perm[index]) {status = GT; break;}
	    if(perm[i] > perm[index]) {status = LT; break;}
            i--;
	 }
      }
      counter++;
      for(int i=0; i<total; i++) printf(perm[i] == L ? "L" : perm[i] == R ? "R" : perm[i] == S ? "S" : "");
      printf(status == EQ ? " MIR" : status == GT ? " GT" : "");
      printf("\n");
   }
}
